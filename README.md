...
**英語見出しの日本語訳併記:** 見出し文が日本語を含まない(=英語などの)場合は、見出しの下に
機械翻訳した日本語訳を小さめの文字で併記している。Google翻訳の無料エンドポイントはGitHub
Actionsの共有IPからだとレート制限(HTTP 429)で確実に失敗するため、サーバー側の一括翻訳では
なく各閲覧者のブラウザから直接呼び出す方式にしている(同エンドポイントはCORSが開放されており
ブラウザから呼び出し可能)。ページ表示自体は原文見出しのまま即座に行われ、日本語訳は表示後に
バックグラウンドで後追い挿入される。翻訳結果はブラウザのlocalStorageにキャッシュされ、再訪問時は
再翻訳しない。1ページに見出しが数百件あるため、翻訳エンドポイント側のレート制限に閲覧者のIPでも
達することがあり、検知した時点でそのページ表示分の翻訳は打ち切る(再読み込みすれば再度試みる)。
...
- **見出しの日本語訳は機械翻訳のベストエフォート**: Google翻訳の非公式エンドポイントを
  ...
  GitHub Actionsの共有IPがレート制限〈HTTP 429〉されるため断念。実際にこのプロジェクトで
  検証し、共有IPからは1件目から429で拒否されることを確認した)。仕様変更や閲覧環境により
  翻訳できない場合があり、その場合は日本語訳を省略し原文の見出しのみ表示する(記事本文
  までは翻訳していない)。
